...
_do_uninstall=0   # set to 1 by --uninstall flag

_parse_vpu_flags_from_rem_args() {
  # Parse from REM_ARGS (provided by parse_common_cli in function.sh).
  # We do NOT mutate REM_ARGS because autorun_setup reuses it.
  local i=0
...
# ---- --uninstall: 停用並移除 service，清除 counter，立即退出 ----
if [[ "${_do_uninstall}" -eq 1 ]]; then
  _entry_full="$(readlink -f "${BASH_SOURCE[0]:-$0}")"
  log_dir "" 1 2>/dev/null || true   # 需要 _log_dir 讓 autorun_uninstall 清 counter
  autorun_uninstall "${_entry_full}"
  exit 0
fi
